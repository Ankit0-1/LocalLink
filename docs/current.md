...
Per `TASKS.md`, Phase 1 backend and frontend login/register views are both done. Remaining Phase 1 frontend work: `RequireAuth`/`RequireRole` route guards (the login page already reads `location.state.from` in anticipation of `RequireAuth` setting it). Each phase in `TASKS.md` now has an explicit Definition of Done checklist to verify against before moving to the next phase.
